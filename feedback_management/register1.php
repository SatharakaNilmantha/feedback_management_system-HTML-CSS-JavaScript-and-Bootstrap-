...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$error) 
    {
        // Check if the user details match an entry in the student table
        $Statement = $connection->prepare("SELECT Id FROM student WHERE User_Name = ? AND Password =? AND First_Name = ? AND Last_Name = ?");
        if ($Statement) 
        {
         ...
         // Check if the user details  in the student_register table
         $Statement = $connection->prepare("SELECT id FROM student_register WHERE User_Name = ? AND Password =? AND First_Name = ? AND Last_Name = ?");
           if ($Statement) 
           {
                    $Statement->bind_param('ssss', $User_Name,$Password, $First_Name, $Last_Name);
                    $Statement->execute();
                    ...
                    if ($Statement->num_rows != 0) 
                    {
                        $User_Name_error = "User alrady registerd ";
                        $error = true;
                    }
                    $Statement->close();
           } 
        ...
        if (!$error) 
        {
            ...
            // Insert into student_register table
            $Statement = $connection->prepare("INSERT INTO student_register (First_Name, Last_Name, User_Name, Password) VALUES (?, ?, ?, ?)" );
            if ($Statement) {
                ...
                header('location:/feedback_management/loging1.php');
                exit;
            } else {
                die("Prepare statement failed: " . $connection->error);
            }
        ...
        <div class=" row container my-3 all "style="margin-bottom:50px">
              ...
              <div class=" form-style container border col-5 ms-5 py-3" id="myForm" style="box-shadow: 5px 10px 18px #888888;">
                  ...
                  <form  class="form-container" method="post" >
                   ...
                   <div class="py-3 row my-1"></div>

                    <a class="col-6 ms-5 me-5" href="loging1.php">Back</a>
