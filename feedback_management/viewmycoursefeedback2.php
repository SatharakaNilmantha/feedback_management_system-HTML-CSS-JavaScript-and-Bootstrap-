...
// Default query to display all feedbacks for the logged-in user
    $sql = "SELECT 
            ...
            INNER JOIN 
                course_feedback cf ON lr.First_Name = SUBSTRING_INDEX(cf.Lecture_Name, ' ', 1) 
                    AND lr.Last_Name = SUBSTRING_INDEX(cf.Lecture_Name, ' ', -1)
            WHERE 
                lr.User_Name = ?
            ORDER BY 
                cf.Semester ASC, cf.Course_Code ASC";
    ...
    $sql = "SELECT 
        ...
        INNER JOIN 
            course_feedback cf ON lr.First_Name = SUBSTRING_INDEX(cf.Lecture_Name, ' ', 1) 
                AND lr.Last_Name = SUBSTRING_INDEX(cf.Lecture_Name, ' ', -1)
        WHERE 
            lr.User_Name = ? AND (cf.Semester LIKE ? OR cf.Course_Code LIKE ? OR cf.Course_Name LIKE ?)
        ORDER BY 
            cf.Semester ASC, cf.Course_Code ASC";
                        ...
                        if ($result->num_rows > 0)
                        {
                            while($row = $result->fetch_assoc()) 
                            {
                                ...
                                <tr>
                                    ...
                                    <td>{$row['Date_Time']}</td>
                                    <td>
                                        <a class='btn btn-primary' href='viewcoursefeedback2.php?id={$row['Id']}'>View</a>
                                    </td>
                                </tr>
                                ";
                             } 
