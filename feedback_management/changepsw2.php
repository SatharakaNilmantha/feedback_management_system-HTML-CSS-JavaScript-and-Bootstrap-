...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $sql = "SELECT * FROM lecture_register WHERE User_Name = '$User_Name';";
    $result = $connection->query($sql);
    $row = $result->fetch_assoc();

    $sql1 = "SELECT * FROM lecture WHERE User_Name = '$User_Name';";
    ...
    do {
        ...
        $sql = "SELECT * FROM lecture_register WHERE User_Name = '$User_Name' AND Password = '$Password';";
        $result = $connection->query($sql);
        ...
        $sql1 = "SELECT * FROM lecture WHERE User_Name = '$User_Name' AND Password = '$Password';";
        $result1 = $connection->query($sql1);
        ...
        // Update the password
        $sql = "UPDATE lecture_register SET Password = '$New_Password' WHERE User_Name = '$User_Name';";
        $result = $connection->query($sql);
        if (!$result) {
        ...
        $sql1 = "UPDATE lecture SET Password = '$New_Password' WHERE User_Name = '$User_Name';";
        $result1 = $connection->query($sql1);
        if (!$result1) {
            $errorMessage = "Invalid query: " . $connection->error;
            break;
        ...
        header("location:/feedback_management/index2.php");
        break;
    } while (false);
}
?>
