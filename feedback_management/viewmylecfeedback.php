...
$sql = "SELECT Id, Semester, Lecture_Name, Course_Code, Date_Time FROM lecture_feedback WHERE User_Name = ?";
...
if (isset($_POST['submit']) && !empty($_POST['search']))
{
    ...
    $sql = "SELECT Id, Semester, Lecture_Name, Course_Code, Date_Time FROM lecture_feedback 
            WHERE User_Name = ? AND (Semester LIKE ? OR Lecture_Name LIKE ? OR Course_Code LIKE ?)";
   ...
   <body class="body">
            ...
            <nav class="navbar navbar-expand-sm  navbar-dark container nav" style="background-color:#07075ca7;height: 80px;">
                    ...
                    <form class="d-flex col-3" method="post" action="">
                      <input class="form-control me-2" type="search" placeholder="Search by Lec_Name/C_Name/Sem" name="search" aria-label="Search">
                                ...
                                while ($row = $result->fetch_assoc())
                                 {
                                    ...
                                    <tr>
                                        ...
                                        <td>{$row['Date_Time']}</td>
                                        <td>
                                            <a class='btn btn-primary' href='viewlecfeedback1.php?id={$row['Id']}'>View</a>
                                            <a class='btn btn-success' href='editlecfeedback.php?id={$row['Id']}'>Edit</a>
                                            <a class='btn btn-danger col-3' href='deletelecfeedback1.php?id=$row[Id]' onclick='return confirmDelete();'>Delete</a>
                                        </td>
                                    </tr>
                                    ";
                                }
