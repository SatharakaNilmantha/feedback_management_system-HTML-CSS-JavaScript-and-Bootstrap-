...
if (isset($_SESSION["User_Name"])) {
    header('location:/feedback_management/loging2.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$error) {
        // Check if the user details match an entry in the lecture table
        $Statement = $connection->prepare("SELECT Lecture_Id FROM lecture WHERE User_Name = ? AND Password=? AND First_Name = ? AND Last_Name = ?");
        if ($Statement) 
        {
         ...
         // Check  the user details  in the lecture_register table
         $Statement = $connection->prepare("SELECT id FROM lecture_register WHERE User_Name = ? AND Password =? AND First_Name = ? AND Last_Name = ?");
           if ($Statement) 
           {
                    $Statement->bind_param('ssss', $User_Name,$Password, $First_Name, $Last_Name);
                    $Statement->execute();
        ...
        if (!$error) 
        {
            ...
            // Insert into lecture_register table
            $Statement = $connection->prepare("INSERT INTO lecture_register (First_Name, Last_Name, User_Name, Password) VALUES (?, ?, ?, ?)");
            if ($Statement) 
            {
              ...
              <div class=" form-style container border col-5 ms-5 py-3" id="myForm" style="box-shadow: 5px 10px 18px #888888;">
                  ...
                  <form  class="form-container" method="post" >
                   ...
                   <div class="py-3  my-1"></div>
                    ...
                    <a class="col-6 ms-5 me-5" href="loging2.php">  Back </a>
